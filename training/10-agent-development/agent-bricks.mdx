...
Genie는 자연어 질문을 **SQL 쿼리로 변환** 하여 데이터에서 답변을 찾아주는 에이전트입니다. 비개발자도 "이번 달 서울 매출이 얼마야?"라고 물으면, 자동으로 SQL을 생성하고 실행하여 결과를 보여줍니다. Agent Bricks의 Genie는 [08. AI/BI 섹션의 Genie](../08-ai-bi/genie)와 동일한 기술을 에이전트 형태로 제공합니다.
