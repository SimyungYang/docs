...
> **참고**
**중요**: 많은 오픈소스 MCP 서버가 stdio만 지원합니다. Databricks Genie Code에서 사용하려면 Streamable HTTP를 지원하는 서버를 선택하거나, Custom MCP 서버로 래핑해야 합니다. [인기 서버 & 활용 시나리오](popular-servers)에서 각 서버의 지원 전송 방식을 확인하세요.
...
| 가이드 | 내용 |
|--------|------|
| [일반 MCP 설정](setup-general) | Claude Desktop, Claude Code, Cursor에서 MCP 서버 설정하기 |
| [인기 서버 & 활용 시나리오](popular-servers) | 카테고리별 인기 MCP 서버, 실전 자동화 시나리오 10선 |
| [Databricks MCP 활용](databricks-mcp) | Databricks 환경에서 Managed/External/Custom MCP 활용하기 |
| [베스트 프랙티스](best-practices) | 보안, Tool 설계, 에러 핸들링, 디버깅 팁 |
