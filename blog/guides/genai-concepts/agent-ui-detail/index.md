...
| 페이지 | 설명 |
|--------|------|
| [Streamlit & Gradio](streamlit-gradio) | Python 기반 빠른 프로토타이핑 도구. 채팅 UI 구현, 스트리밍, 비교 |
| [Chainlit & Dash](chainlit-dash) | Agent 추론 시각화 전용 UI(Chainlit), 데이터 대시보드 특화(Dash) |
| [FastAPI 백엔드](fastapi-backend) | REST API, SSE 스트리밍, WebSocket 양방향 통신, Frontend 분리 아키텍처 |
| [Databricks Apps & 종합 가이드](databricks-apps) | Databricks Apps 배포, 단계별 기술 스택 선택, 종합 비교, 고객 FAQ, 연습문제 |
...
다음 페이지: [Streamlit & Gradio](streamlit-gradio)
